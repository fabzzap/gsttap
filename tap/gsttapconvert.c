...
#define DEBUG_INIT(bla) \
  GST_DEBUG_CATEGORY_INIT (gst_tapconvert_debug, "tapconvert", 0, "TAP rate convert");

GST_BOILERPLATE_FULL (GstTapConvert, gst_tapconvert, GstBaseTransform,
    GST_TYPE_BASE_TRANSFORM, DEBUG_INIT);

static gboolean gst_tapconvert_set_caps (GstBaseTransform *trans, GstCaps *incaps,
                                 GstCaps *outcaps);
...
static void
gst_tapconvert_base_init (gpointer klass)
{
 ...
 */
static void
gst_tapconvert_init (GstTapConvert *filter, GstTapConvertClass * klass)
{
}
...
gst_tapconvert_transform_ip (GstBaseTransform * base, GstBuffer * outbuf)
{
  ...
  guint buflen = GST_BUFFER_SIZE(outbuf) / sizeof(int32_t);
  guint bufsofar;
  guint *data = (guint*) GST_BUFFER_DATA(outbuf);
  ...
  for (bufsofar = 0; bufsofar < buflen; bufsofar++) {
    ...
    data[bufsofar] = (guint) (pulse / filter->inrate);
  }
                                 ...
                                 GstCaps *outcaps)
{
  ...
  gboolean ret2 = gst_structure_get_int (outstructure, "rate", &filter->outrate);
                                   ...
                                   GstCaps *caps)
{
  ...
  GstPad * peer = gst_pad_get_peer (otherpad);
  ...
  GST_DEBUG_OBJECT (trans, "to: %" GST_PTR_FORMAT
  , newstructure);
